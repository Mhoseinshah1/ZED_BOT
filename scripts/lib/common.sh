...
# String.prototype.trim), byte-for-byte and locale-independently. WITHOUT eval,
# without logging/printing it, and without exposing its length/prefix/suffix/hash
# - interior characters are left untouched. Returns the trimmed value via stdout
...
# inline copy; a source-parity test keeps the two function bodies verbatim-equal.
