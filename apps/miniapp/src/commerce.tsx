...
export function BuyScreen(props: {
  ...
  if (props.view.kind === "hub") return <Card title={C.buyTitle}><div className="actions">
    <button type="button" className="button" onClick={() => props.onView({ kind: "catalog", mode: "service" })}>{C.buySubscription}</button>
  ...
  if (props.view.kind === "catalog") return <Catalog mode={props.view.mode} back={() => props.onView({ kind: "hub" })} select={(product, panel) => props.onView({ kind: "purchase", product, panel })} />;
  if (props.view.kind === "purchase") return <Purchase product={props.view.product} panel={props.view.panel} back={() => props.onView({ kind: "catalog", mode: "service" })} paid={paymentPublicId => props.onView({ kind: "payment", paymentPublicId })} />;
  return <PaymentStatus paymentPublicId={props.view.paymentPublicId} back={() => props.onView({ kind: "hub" })} />;
}

function Catalog(props: { mode: "service" | "pricing"; back: () => void; select: (product: CatalogProductDto, panel: CatalogPanelDto) => void }) {
  ...
  return <div className="stack"><button type="button" className="button button--ghost" onClick={props.back}>{C.back}</button>
    {state.data.servicePanels.map(panel => <Card key={panel.publicId} title={panel.name}>
      {panel.categories.map(category => <section key={category.publicId}><h3>{category.name}</h3>
        {category.products.map(product => <button key={product.publicId} type="button" className="txn" disabled={props.mode === "pricing"} onClick={() => props.select(product, panel)}>
...
function AuthoritativeQuote(props: { quote: QuoteDto; busy: boolean; pay: () => void }) {
  return <Card title={C.preInvoice}>
    ...
    <button type="button" className="button" disabled={props.busy || !props.quote.walletPayEnabled} onClick={props.pay}>{C.payWithWallet}</button>
  </Card>;
}

function Purchase(props: { product: CatalogProductDto; panel: CatalogPanelDto; back: () => void; paid: (id: string) => void }) {
  ...
  return <div className="stack"><button type="button" className="button button--ghost" onClick={props.back}>{C.back}</button>
    {quote === null ? <Card title={C.usernameStep}>
      ...
      <button type="button" className="button" disabled={busy || (mode === "CUSTOM" && username.trim() === "")} onClick={() => void makeQuote()}>{C.preInvoice}</button>
    </Card> : <AuthoritativeQuote quote={quote} busy={busy} pay={() => void pay()} />}
...
export function AddonsSection(props: { servicePublicId: string; onPayment: (id: string) => void }) {
  ...
  if (state.phase === "loading") return <Spinner />;
  if (state.phase === "failed") return state.failure.code === "FEATURE_DISABLED" ? null : <Failure failure={state.failure} retry={reload} />;
  if (quote?.ok) return <div className="stack"><AuthoritativeQuote quote={{ ...quote.quote, productName: quote.quote.productName, panelName: null, note: null, discountStackingRejected: false, needsCustomerInputBeforePayment: false }} busy={locked.current} pay={() => {
