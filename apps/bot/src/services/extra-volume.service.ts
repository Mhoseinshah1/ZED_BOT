...
export async function listExtraVolumeServices(
  ...
  const services = eligible.slice(
    (safePage - 1) * EXTRA_VOLUME_PAGE_SIZE,
    safePage * EXTRA_VOLUME_PAGE_SIZE,
  );
...
export async function getExtraVolumeServiceByShortId(
  ...
  // Legacy per-inbound XUI services never take extra volume through the
...
async function executeExtraVolumeOrderUnlocked(
  ...
  if (!panelResult.ok && panelResult.uncertain === true) {
    ...
    // positive proof.
    logger.error("extra volume panel outcome unknown - deferring to reconciliation", {
      orderId: order.id,
      serviceId: service.id,
      panelId: panel.id,
