import {
  ...
  lockForceJoinConfig,
  setForceJoinSettingCacheInvalidator,
...
// The bot memoizes Setting reads, so the shared policy has to be able to drop
// `force_join_enabled` when retiring a channel flips it. Installed here, at the
// module every bot force-join caller already imports.
setForceJoinSettingCacheInvalidator(clearSettingCacheKeys);
...
// The master switch key, the config advisory lock, the deterministic ordering,
// the bounded health policy and the active-channel read now live in
// `@zedbot/force-join` so the API can enforce the identical gate without
// importing grammY. They are re-exported unchanged: every existing caller and
// test in this app keeps importing them from here.
export {
  CHANNEL_ORDER_BY,
  FORCE_JOIN_CONFIG_LOCK,
  FORCE_JOIN_ENABLED_KEY,
  FORCE_JOIN_HEALTH_COUNT_DEBOUNCE_MS,
  FORCE_JOIN_HEALTH_FAILURE_THRESHOLD,
  FORCE_JOIN_HEALTH_MIN_WINDOW_MS,
  FORCE_JOIN_HEALTH_RESET_MS,
  listActiveChannels,
  lockForceJoinConfig,
  recordChannelHealthFailure,
  recordChannelHealthSuccess,
  recordValidationError,
  type HealthFailureOutcome,
} from "@zedbot/force-join";
