...
// =============================================================================
// Navigation-tree integrity (corrective UI/UX audit). Two structural locks
// over the WHOLE bot, so the tree in docs/navigation-map.md cannot silently
// rot:
//   1. every callback a keyboard can emit has a registered handler
//      (no dead buttons anywhere, user or admin);
//   2. every rendered page passes a keyboard (no dead-end pages).
// Static source analysis - no database, no Telegram.
// =============================================================================
...
const CALLBACK_NS = "(?:user|admin|common|terms|force_join)";

/** IDENT.prop -> "literal" map values defined in one file. */
function literalConsts(src: string): Map<string, string> {
  const map = new Map<string, string>();
  const re = new RegExp(`([A-Za-z0-9_]+):\\s*"(${CALLBACK_NS}:[a-z0-9_:.]+)"`, "g");
...
describe("navigation integrity", () => {
  ...
  it("every emitted button callback has a registered handler (no dead buttons)", () => {
    ...
    for (const [file, src] of sources) {
      const templateRe = new RegExp("`(" + CALLBACK_NS + ":[a-z0-9_:.]*?)\\$\\{", "g");
      for (const match of src.matchAll(templateRe)) {
        emitted.set(match[1], file);
      }
      const literalRe = new RegExp('"(' + CALLBACK_NS + ":[a-z0-9_:.]+)\"", "g");
      for (const match of src.matchAll(literalRe)) {
        emitted.set(match[1], file);
      }
    }
    expect(emitted.size).toBeGreaterThan(200); // the tree really was scanned

    // Registered handlers: regex routes, string routes, and constant refs
    // resolved against the same-file callback map or the core CB map.
    ...
    expect(regexes.length).toBeGreaterThan(100);
  ...
  it("the navigation map documents the locked flows", () => {
    const map = readFileSync(path.join(repoRoot, "docs/navigation-map.md"), "utf8");
    expect(map).toContain("خرید اشتراک");
    expect(map).toContain("محصولات دیگر");
    expect(map).toContain("LOCKED");
    expect(map).toContain("zero dead buttons");
  });
});
