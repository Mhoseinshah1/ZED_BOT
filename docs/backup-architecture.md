...
1. `[1/14]` safety archive (`.env` + database, `scripts/backup.sh`).
2. `[2/14]` `backup-db.sh`, then the newest backup is verified per type:
...
3. Only then the remaining steps run: pull → `.env` migration → CLI
   ...
   [legacy-upgrade.md](legacy-upgrade.md) for the full 14-step flow.
