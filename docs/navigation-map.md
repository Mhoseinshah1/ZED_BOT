...
| detail (`user:svc:view:<sid>`) | بروزرسانی اطلاعات ♻️ · لینک اشتراک 🔗 · QR اشتراک 📷 · کانفیگ‌ها 📄 · QR کانفیگ‌ها 📷 · تغییر لینک اشتراک 🔄 (»confirm) · **آموزش اتصال 📱 » `user:svc:guide:<sid>` (when the guide gate passes)** · **بررسی مشکل سرویس 🛠 » `user:svc:diag:<sid>` (when `service_diagnostics_enabled`)** · **تمدید سرویس ♻️ » `user:renew:svc:<sid>` (Fix A, when renewable)** · خرید حجم اضافه ➕ » `user:ev:svc:<sid>` · خرید زمان اضافه ⏳ » `user:et:svc:<sid>` · خاموش/روشن کردن سرویس (»confirm) · بازگشت به لیست · بازگشت به منوی اصلی |
...
| service diagnostics report (`user:svc:diag:<sid>`, when `service_diagnostics_enabled`) | recommended actions » existing routes (link/QR/configs/guide/enable/renew/extra/regen) · بررسی دوباره 🔄 » `user:svc:diag:<sid>:retry` · ارسال گزارش به پشتیبانی 🎫 » `user:svc:diag:<sid>:support` · بازگشت به سرویس · بازگشت به منوی اصلی |
| diagnostics support preview (`user:svc:diag:<sid>:support`) | safe snapshot summary · confirm » `user:svc:diag:<sid>:sup_yes` (arms the existing support MESSAGE flow + attaches the validated snapshot) · بازگشت به سرویس |
...
| product detail | field edits (incl. پیام تکمیل سفارش » `admin:prod:fe:<sid>:cmt`, OTHER only) · category/groups · (SERVICE) پنل/حجم/موقعیت · (SERVICE, **OWNER**) فروش نمایندگی » `admin:prod:repel:<sid>` (confirm page » `admin:prod:repel:<sid>:<0\|1>`, the digit is the expected current state; one atomic conditional flip; see `docs/representative-program.md` §8) · (OTHER) نوع محصول » `admin:prod:kind:<sid>` (» `admin:prod:setkind:<sid>:<APPLE\|AI\|TGP\|GIFT\|GEN>`) · فرمت موجودی » `admin:prod:sparser:<sid>` (» `admin:prod:setsp:<sid>:<SL\|SEP\|EB>`, stock profiles only) · دریافت اطلاعات قبل از تایید رسید » `admin:prod:cba:<sid>` (info-collecting products only) · تحویل/اطلاعات/استاک 🎟 » `admin:stock:p:<sid>` · toggle · غیرفعال‌سازی (soft) · بازگشت به لیست (same filter/page) · بازگشت به مدیریت محصولات |
